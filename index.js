...
for (const file of commandFiles) {
	...
	if ("data" in command && "execute" in command) {
		...
		console.log(`[WARNING] La commande à ${filePath} manque d'une propriété "data" ou "execute".`);
	}
}
...
client.once("clientReady", async (readyClient) => {
	...
	try {
		console.log(`Début du rafraîchissement des ${commandsData.length} comme Slash globales...`);
...
client.on("interactionCreate", async (interaction) => {
	...
	try {
		...
		if (interaction.replied || interaction.deferred) {
			await interaction.followUp({ content: "Une erreur est survenue lors de l'execution !", flags: MessageFlags.Ephemeral });
		} else {
			await interaction.reply({ content: "Une erreur est survenue lors de l'execution !", flags: MessageFlags.Ephemeral });
		}
	}
});
